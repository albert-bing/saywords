				...
				.ready(
					function() {
						...
						next_page.onclick = function() {
							...
							var flag = chenck_finish(pdata,a1,a2);
							if(flag == 0){
								chenck_finish(pdata,a1,a2);
							}else{
								anwser_A();
								if(limit_page_number>1){
									layer.open({
								        ...
								        ,offset: 'auto' 
								        ,id: 'layer'+1 
								        ,content: '<div style="padding: 20px 100px;">已经是最后一页了！</div>'
								        ,btn: '确认'
								        ,btnAlign: 'c' 
								        ,shade: 0.5
								        ,yes: function(){
			...
			for(var i=0; i<28; i++){
				...
				if(pdata[i].ans==0){
					...
					layer.open({
				        ...
				        ,offset: 'auto' 
				        ,id: 'layer'+1 
				        ,content: '<div style="padding: 20px 10px;text-align:center;line-height:30px;">还有题目未完成！</div>'
				        ,btn: '关闭全部'
				        ,btnAlign: 'c' 
				        ,shade: 0.5
				        ,yes: function(){
					...
					success : function(r) {
						if (r.code == 0) {
							layer.open({
						        ...
						        ,closeBtn:false;
						        ,id: 'layerDemo'+1 
						        ,content: '<div style="padding: 20px 100px;">问题行为（PartOne部分）测试完毕！<br>点击“确认”，进入PartTwo部分。</div>'
						        ,btn: '确定'
						        ,btnAlign: 'c' 
						        ,shade: 0.5 
						        ,yes: function(){
